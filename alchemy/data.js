...
(function (global) {
  ...
  // ---------------------------------------------------------------- 敷地
  const BOARD = {
    // 上限は「1366x768 のノートで、1マスが最小の22pxを切らずに収まる」いっぱいまで。
    // 一筆書きは盤面全体を見て考えるので、画面からはみ出すところまでは広げない。
    w: 9, h: 7, maxW: 36, maxH: 20,
    // 拡張は40回。値段は二段で、18回目までは 1.45倍ずつ上がり、そこから先は 1.12倍。
    // 1.45倍のまま40回伸ばすと最後が5億Gを超えて、盤面ではなく金で詰まる。
    // 遊んでいて通る範囲（18回目まで）の値段は動かさない。
    landBase: 300, landStep: 1.45, landSoft: 18, landStep2: 1.12,
