...
(function () {
  ...
  var P = {
    ...
    pullR: 50,         // 引き合いが届く距離。0 で無効
    pullF: 120,        // 引き合う強さ。1秒に向きを何ラジアン寄せられるか
    ...
    fusePay: 1.0,      /* 融合そのものに入る点。小さい方の大きさにかける。
                          大物に小物をくっつけても伸びないようにするため。
                          8と1なら1点、8と8なら8点。連鎖倍率は乗せない */
    ...
    comboMin: 0.25,    /* 連鎖として数える最小の間隔（秒）。
                          盤面が一気に崩れるときの融合は 0.1秒刻みの連射になる。
                          そのまま数えると連鎖が伸びすぎて倍率が跳ねる */
  ...
  /* 進行の段。at はスコア。
     ...
     通した人ほど段が進む。融合回数で進めると、同色は勝手に引き合うぶん
     放置でも増えてしまい、何もしない方が速く進む。
  ...
  /* ---------- 光の切り取り ----------
     ...
     そこで「ぼかすのはこの範囲だけ」と切り取ってから描く。絵は変わらない。
  ...
  /* 計測用の切り替え。?tune=1 の「計測」ボタンが順に切り替えて描画の時間を測る。
     どの塗りが重いのかを機械的に潰すためのもの */
