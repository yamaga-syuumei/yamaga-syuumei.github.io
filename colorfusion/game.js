...
(function () {
  ...
  var P = {
    ...
    fusePay: 1.0,      /* 融合そのものに入る点。小さい方の大きさにかける。

                          狙いは「大物に小物をくっつけても得しない」こと。
                          点が小さい方で決まるので、8と1を繋いでも1点、8と8なら8点。
                          拾い食いは何度やっても1点のまま。

                          「できたサイズ」にかけると逆になる。融合の回数が多いぶん、
                          1つずつ足していく方が合計で上回る（16を作ると 135 対 64）。
                          小さい方なら 15 対 32 で、揃えて組んだ方が上。

                          「a×b」も駄目だった。あれは繋ぐ順番によらず合計が同じになる
                          （元の粒どうしの組み合わせが1回ずつ数えられるだけなので）。

                          連鎖倍率は乗せない。乗せると大物と連鎖の掛け算になって
                          ばらつきが戻る（comboMin で抑えたばかりのもの）。

                          なお、この点はバランスの順位を動かさない（実測）。
                          スコアだけ増えてもコアの光は増えないので、
                          難易度が早く来るだけで生存は伸びないため。
                          残しているのは手応えのためで、バランスの道具ではない */
  ...
  function fuse(a, b) {
    ...
    /* 小さい方の大きさ。この下で a.size を書き換えるので、先に取っておく。
       減る量にも融合の点にも使う */
    var small = Math.min(a.size, b.size);
    var lost = small * P.shrink;
    ...
    /* 融合そのものの点。小さい方で決まるので、拾い食いでは伸びない */
    if (P.fusePay > 0) {
